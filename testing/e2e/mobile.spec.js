...
// mobile-only layout (toolbar hidden, X-delete shown, browse overlay) that
// jsdom cannot. Touch GESTURES (pinch / double-tap) are intentionally NOT
// covered here — pinch has no native Playwright support and both are
// timing-sensitive; they are already covered by testing/v3_mobile_gestures.test.js.
...
async function mockMemeTemplateImage(page) {
  await page.route('https://i.imgflip.com/*-test.jpg', route => {
    route.fulfill({ status: 200, contentType: 'image/jpeg', body: templateImageBuffer });
  });
}

// This spec only makes sense on a touch/mobile viewport. The playwright.config
// "mobile-chromium" project emulates an iPhone 13; on the desktop project the
// mobile-only CSS never applies, so skip there instead of failing.
...
test('boots into the browse overlay, loads a meme, and shows the mobile editor layout', async ({ page }) => {
  ...
  // 1. On mobile the app boots into the Browse Memes overlay (app.js calls
  //    showBrowseView() on load). The back button is hidden — no image yet.
  await expect(page.locator('body')).toHaveClass(/browse-open/);
  await expect(page.locator('#mobile-home-back-btn')).toBeHidden();

  // 2. Pick a mocked search result → editor view. browse-open is removed and
  //    the mobile action row (Share visible, Download hidden) is shown.
...
test('selecting a text box applies the mobile layout (toolbar hidden, X-delete shown)', async ({ page }) => {
  await page.goto('/');
  ...
  // The media-query-driven mobile layout — the part jsdom cannot evaluate:
  //   • the bottom text toolbar is hidden,
  //   • the mobile X delete button is shown,
  //   • the Browse/Share row is hidden while a box is selected (styles.css:1184).
  await expect(textBox.locator('.text-box-toolbar')).toBeHidden();
  await expect(textBox.locator('.mobile-delete-btn')).toBeVisible();
...
test('Browse Memes reopens the overlay and back returns to the editor', async ({ page }) => {
  ...
  // Load a meme so we land in the editor with an image (back button needs one).
  await page.locator('#meme-search-input').fill('drake');
  await page.locator('#meme-search-results .meme-search-card[title="Drake Hotline Bling"]').click();
  await expect(page.locator('#canvas-container')).toHaveClass(/has-image/);
  await expect(page.locator('body')).not.toHaveClass(/browse-open/);

  // Tap Browse Memes → overlay opens. (No box selected, so the row is visible.)
  await page.locator('#browse-memes-btn').click();
  await expect(page.locator('body')).toHaveClass(/browse-open/);

  // Back button is now available (image is loaded) and returns to the editor.
