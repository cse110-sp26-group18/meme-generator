...
MemeGen.Exporter = (function () {
  ...
  function getMemeBlob(canvas, ctx, image, textBoxes, callback) {
    ...
    // Re-apply detected-text covers (from the Scan Text OCR flow) so the export
    // matches the editing view and the original baked-in text stays hidden
    // behind the new text. No covers → no-op.
    if (
      MemeGen.Inpaint &&
