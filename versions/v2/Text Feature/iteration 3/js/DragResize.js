...
MemeGen.DragResize = (function () {
  ...
  function attach(textBox) {
    ...
    // setPointerCapture routes all subsequent pointer events to moveBtn even
    // when the pointer leaves the element, replacing the old document-level
    // mousemove/mouseup pattern. Works for both mouse and touch input.
    moveBtn.addEventListener('pointerdown', function (e) {
      ...
      // No preventDefault here so the mousedown event still bubbles to el,
      // which triggers onSelect and keeps the selection logic working.
    });
    ...
    // Pointer capture is also released automatically on pointercancel
    // (e.g. the browser interrupts the gesture), so no pointercancel
    // handler is needed.
    ...
    document.addEventListener('mousemove', function (e) {
      ...
      // Keep live font size in sync with the export formula so WYSIWYG
      textBox.syncFontSize();
    });
